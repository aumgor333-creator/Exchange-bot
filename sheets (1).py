...
def get_all_rows():
    """Возвращает список всех строк курсов как есть (для отображения клиенту),
    каждая строка: {'pair': str, 'rate': float|None, 'from': float|None, 'to': float|None}."""
    client = _get_client()
    sheet = client.open_by_key(SHEET_ID).worksheet(WORKSHEET_NAME)
        ...
        result.append(
            {
                "pair": pair,
                "rate": _to_float(row[COL_RATE]),
                "from": _to_float(row[COL_FROM]),
                "to": _to_float(row[COL_TO]),
            }
        )
    return result
