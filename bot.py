...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if stage == "currency":
        ...
        await update.message.reply_text(
            "Укажите сумму для обмена, пожалуйста."
        )
        await update.message.reply_text(
    ...
    if stage == "amount":
        ...
        if pair == "RUB/VND" and amount >= 50000:
            lines.append(
                "* Напоминание: у нас действует прогрессивная шкала — чем больше "
                "сумма (от 50 000 и от 100 000), тем выгоднее курс."
            )
